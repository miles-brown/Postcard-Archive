...
export async function createPostcard(postcard: InsertPostcard) {
  ...
  const result = await db.insert(postcards).values(postcard).returning({ insertedId: postcards.id });
...
export async function createPostcardImage(image: InsertPostcardImage) {
  ...
  const result = await db.insert(postcardImages).values(image).returning({ insertedId: postcardImages.id });
  return result[0].insertedId;
}
...
export async function createTranscription(transcription: InsertTranscription) {
  ...
  const result = await db.insert(transcriptions).values(transcription).returning({ insertedId: transcriptions.id });
  return result[0].insertedId;
}
...
export async function createScrapingLog(log: InsertScrapingLog) {
  ...
  const result = await db.insert(scrapingLogs).values(log).returning({ insertedId: scrapingLogs.id });
  return result[0].insertedId;
}
